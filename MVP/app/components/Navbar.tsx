  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 pt-[clamp(12px,1.05vw,20px)]">
      <nav
        ...
        className="mx-auto flex h-[clamp(58px,4.2vw,80px)] w-[min(calc(100%-24px),802px)] items-center justify-between rounded-[999px] bg-white pl-[clamp(18px,1.58vw,30px)] pr-[4px] shadow-none"
      >
        <Link
          href="/"
          aria-label="Cognis AI home"
          className="flex h-full w-[clamp(74px,8.42vw,160px)] items-center rounded-full transition-opacity hover:opacity-75"
        >
          <Image
            src="/logo/logo.svg"
            alt=""
        ...
        >
          ...
          {navLinks.map((link, index) => {
            const isActive =
              link.href === "/about" ? pathname === "/about" : pathname === "/" && index === 0;
        ...
        <MovingBorderLink
          href="/#features"
          className="h-[clamp(50px,3.55vw,68px)] w-[clamp(164px,11.7vw,224px)]"
